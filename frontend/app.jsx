console.log("Executing app.jsx...");

const { useState, useEffect, useMemo } = React;
console.log("React hooks loaded.");
const { LineChart, Line, BarChart, Bar, ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ZAxis, Cell } = window.Recharts;
console.log("Recharts loaded.");

const db = window.OCEANIQ_DATA;
...
const Navbar = () => (
    <nav className="fixed top-0 w-full bg-ocean-navy/90 backdrop-blur-md border-b border-ocean-teal z-[100] px-6 py-4 flex justify-between items-center">
        ...
        <div><span className="bg-ocean-teal text-white px-3 py-1 rounded text-xs tracking-widest uppercase">Team Corals • Smartathon 2.0</span></div>
    </nav>
);
...
const Section4 = () => {
    ...
    return (
        <SectionWrapper id="calculator">
            ...
            <p className="text-gray-400 mb-8 italic">Team Corals Innovation: A physics-based formula triggering biological warnings.</p>
...
const Section5Map = () => {
    ...
    React.useEffect(() => {
        ...
        const map = window.L.map(mapRef.current, { scrollWheelZoom: false }).setView([17.0, 79.0], 5);
        ...
        db.overview.forEach(z => {
            ...
            marker.bindPopup(`
                <div class="p-1">
                    ...
                    <p class="text-xs mt-1 text-gray-400">Rainfall: ${z.rainfall}mm</p>
                    <p class="text-xs text-gray-400">SST: ${z.sst}°C</p>
                </div>
            `);
        });
...
const Section8 = () => (
    <SectionWrapper id="biodiversity">
        ...
        <p className="text-gray-400 mb-8 italic text-sm">High nutrient pressure zones show significant biodiversity loss, threatening coral reefs and marine ecosystems.</p>
    ...
    return (
        <div className="w-full flex flex-col items-center">
            ...
            <footer className="w-full py-6 text-center bg-ocean-navy border-t border-ocean-teal/30 text-ocean-seafoam/60 text-sm glow-text-seafoam mt-12 z-10">
                OceanIQ AI Platform © 2026 | Built by Team Corals | Smartathon 2.0 (IEEE IAS × L&T)
            </footer>
        </div>
    );
};
